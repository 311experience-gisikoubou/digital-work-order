...
export function validateWipEvidence(evidence, { repo, nowMs }) {
  ...
  return { ok: true, pullRequests: evidence.pullRequests };
}
...
function failClosedReport({ code, repo, generatedAt }) {
  return {
    ok: false, code, repository: repo, generatedAt,
...
export function buildReport({ evidence, repo, nowMs, generatedAt }) {
  ...
  return {
    ok: true, code: null, schemaVersion: 1, repository: repo, generatedAt,

...
for (const entry of entries) {
  ...
  if (!path || !isCanonicalPath(path) || path === 'AGENTS.local.md') {
    fail('FOUNDATION_REMOTE_PLAN_PATH_OUTSIDE_SHARED_SURFACE', { path: entry?.path ?? null });
  }
  ...
  if (oldSha !== null && newSha !== null) {
    ...
    if (oldSha === newSha) {
      ...
      operations.push({ op: 'replace', path, oldSha, newSha, targetSha });
  ...
  if (oldSha === null && newSha !== null) {
    if (targetSha === null) {
      operations.push({ op: 'create', path, oldSha: null, newSha, targetSha: null });
      requiredBlobShas.add(newSha);
      createdCount += 1;
    } else if (targetSha === newSha) {
      unchangedCount += 1;
    } else {
      fail('FOUNDATION_REMOTE_PLAN_NEW_PATH_CONFLICT', { path, newSha, targetSha });
    }
  }
}

const treeElements = operations.map((entry) => ({
  path: entry.path,
  mode: '100644',
  type: 'blob',
  sha: entry.op === 'delete' ? null : entry.newSha,
}));

const output = {
  result: 'PASS',
  code: 'FOUNDATION_REMOTE_UPDATE_PLAN_READY',
  fromVersion,
  sourceVersion,
  fromCommit,
  sourceCommit,
  ...
  requiredBlobShas: [...requiredBlobShas].sort(),
  operations,
  gitDataContract: {

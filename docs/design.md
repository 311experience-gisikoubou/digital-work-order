...
本節は設計境界の正本化のみで、実装・Firebase/Google Cloudの正式採用・課金有効化・暗号方式/ライブラリの最終選定・実データ投入は含まない。第12節・14.1・第13節の「外部クラウドへ送信しない」は現行実装の境界であり、本節の将来設計が別Issueで実装・承認されるまで変更しない。既存機能の挙動は変更しない。

### 15.1 識別子と経路

- 転送・保存・照合の中心IDは既存の `workOrderRef` とする。
- 拠点間転送は「医院iPad → 暗号化一時クラウド → 技工所Windows PC」とする。
- Local Transfer BridgeはLAN内専用とし、拠点間転送と分離する。Bridgeにクラウド経路を持たせず、クラウド経路にLAN前提を持ち込まない。

### 15.2 暗号化・署名

- iPad側で暗号化してから送信する。クラウドには暗号文のみを一時保存し、平文・鍵をクラウドへ置かない。
- 暗号化対象は `work-order.json` / `manifest.json` / 添付メディアのすべてとする。
- 医院iPadが送信署名を付け、技工所PCは登録済み端末の公開鍵で署名を検証する。未登録・失効端末・検証失敗は受領しない（fail closed）。
- 鍵の役割（暗号化用・署名用・端末識別）を分離して定義する。端末紛失時は当該端末を失効し、鍵更新・再ペアリングで復旧する。秘密鍵のバックアップ方針（要否・保管場所・復旧手順）はPhase 5前に確定し、それまで実データ用途へ進めない。
- 暗号方式・ライブラリの最終選定は後続Issueで行う。

### 15.3 受領完了と削除

- iPadの原本は、技工所PCの受領完了（署名・SHA-256照合成功）を確認するまで削除しない。
- 技工所PCの受領完了後にクラウド上の暗号文を削除する。未受領の期限切れは保持期限に従って削除するが、その際もiPad原本は残す。

### 15.4 manifest完全性

`manifest.json` は少なくとも次を持つ: `schemaVersion`, `workOrderRef`, 各メディアの識別子・種別・MIME・サイズ・SHA-256、チャンク構成（チャンク数・サイズ・チャンクごとのハッシュ）。技工所PCはサイズ・SHA-256・チャンク完全性が一致しない場合、受領完了としない。

### 15.5 技工所PC側の保存

- メディア本体はファイルとして保存し、DBにはパス・ハッシュ・種別等のメタ情報だけを保持する。
- PDFにはメディア本体を埋め込まず、メディア件数のみを表示する。
- ZIPは通常運用の経路にせず、非常時・移行用途に限定する。

### 15.5.1 Phase 0で確定しないこと

- iPad側の一時保管（OPFS等）、録音・動画、UI、クラウド、PC受信の実装はPhase 1以降とする。
- Google Cloud / Firebaseの採用可否、リージョン、保持期限、課金、契約、医療情報の取扱いガイドライン適合は、Phase 5直前に人間が確認する。この確認前に課金を有効化せず、実データを扱わない。

### 15.6 Phase 0の完了条件

本節が上記の境界を矛盾なく記録していること、`git diff --check` がPASSすること、アプリコード・既存挙動・実データ・secretsを変更/追加していないこと。

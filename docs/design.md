...
- ブラウザ内前処理は長辺最大1600pxの平滑縮小（拡大なし）、白背景合成、グレースケール、最大1.5倍の保守的コントラスト調整のみ。閾値化・シャープ化・複数OCR passは行わない。
...
- 次の実機ゲートは、印刷した架空指示書をiPad Safariで撮影し、`ocrDebug=1` の集計値だけで「OCR段階で落ちているか／後段の厳格判定で落ちているか」を1回で切り分ける。実患者・実医院・実受注データは使用禁止。詳細は `docs/issue41-verification.md`。PR #42はDraft、Merge authorized NO。

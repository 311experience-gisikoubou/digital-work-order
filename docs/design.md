...
- 2026-09-07のiPad Safari実機確認は、PC画面上の架空テスト指示書を撮影して2回実施した。前処理追加前・追加後ともOCR候補4項目はすべて空欄で、有効性はNG。一方、誤候補の自動反映はなく、人間承認必須も維持されたため安全性/fail-closedは2回ともPASS。
- 2回の解決介入が有効性改善に失敗したため、同じ調整を繰り返さない。PC画面撮影はモアレを生み、本来の紙指示書取り込み条件を代表しないため、次回は印刷した架空指示書を使う。
- 既知ラベルの文字間空白・タブ、CR / CRLF / LF / Unicode行区切りを許容するが、ラベル文字訂正・値推測はしない。明示コロン、信頼度80以上、重複/曖昧拒否、完全な日付、4項目限定を維持する。
- ブラウザ内前処理は長辺最大2400pxの平滑縮小（拡大なし）、白背景合成、グレースケール、最大1.5倍の保守的コントラスト調整のみ。1600→2400は印刷紙の診断結果に基づく1点変更で、信頼度80以上・閾値化なし・シャープ化なし・複数OCR passなしは維持する。
- 原因切り分け用に `ocrDebug=1` のときだけ集計診断を表示する。通常モードのUIは変えない。診断はOCR総行数、信頼度80以上行数、信頼度フィルタ前後の既知ラベル一致数、最終非空候補数、項目別候補有無だけで、認識文字列や候補値は表示・保存しない。
- 診断情報は新規OCR、キャンセル、画像差し替え、明示破棄、pagehide/beforeunloadで消去する。localStorage / sessionStorage / IndexedDB / Cache Storage / state.orders / telemetry / 外部サービスへ保存しない。
- 自動検証は45 PASS / 0 FAIL / 0 skipped / 0 cancelled。診断の非表示既定、集計値のみ、ライフサイクル消去もテストで固定した。外部OCR/AI/API/CDN、新規依存、閾値緩和、曖昧推測、自動承認は追加していない。
- 印刷した架空指示書のiPad Safari診断では、OCR行数9、信頼度80以上1、ラベル一致数は信頼度フィルタ前1・後0、最終候補0だった。したがって現在の主因は後段パーサではなくOCR信頼度不足と判定する。次の実機ゲートは、同じ印刷紙を2400px上限版で1回だけ再診断し、集計値を比較する。実患者・実医院・実受注データは使用禁止。詳細は `docs/issue41-verification.md`。PR #42はDraft、Merge authorized NO。
